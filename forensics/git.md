...
* **Commits**: Save-states of the files in your repository. Whenever you change something and you are happy with the change, you can commit it so it's saved as a snapshot where you can later go back to. Can be created using `git commit -m "message"`
...
All the information about your Git repository gets saved in a `.git` directory that is in the root of your repository. The `git` command interacts with this directory and lots of tools can get information from it. So if you ever find a `.git` directory you'll know the current directory is a Git repository.&#x20;
...
When you visit the `.git` directory on the website, and you can see a list of files relating to git, you know directory listing is on. This makes it really easy to download everything at once recursively and to then examine the repository on your own machine.&#x20;
