...
Native functions are defined in `.so` binaries in the `lib/` folder, you should use binary [Reverse Engineering](https://app.gitbook.com/s/677wrA8ZfiPs1U4l5uR6/reverse-engineering "mention") tools to analyze them. Then, you may find custom functions or even library functions of interest which you want to log the arguments/memory of. While possible to do manually in GDB, the easiest way is by setting a hook with Frida:
